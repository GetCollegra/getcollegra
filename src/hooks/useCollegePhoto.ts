...
const STORAGE_PREFIX = "collegra:photo:v1:";
...
async function fetchPhoto(collegeName: string): Promise<string | null> {
  ...
  const promise = (async () => {
    try {
      ...
      const photos = (data?.photos as Array<{ url?: string; thumbnailUrl?: string }> | undefined) || [];
      const first = photos.find(p => p.url || p.thumbnailUrl);
      const url = first?.url || first?.thumbnailUrl || null;
...
export function useCollegePhoto(collegeName: string | undefined | null) {
  ...
  useEffect(() => {
    ...
    fetchPhoto(collegeName).then(result => {
      if (cancelled) return;
      setUrl(result);
      setLoaded(true);
    });
